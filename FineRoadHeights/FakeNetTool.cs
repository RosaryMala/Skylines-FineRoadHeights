...
namespace FineRoadHeights
{
	public class FakeNetTool
	{
        ...
        public IEnumerator<bool> ChangeElevation(int delta)
        {
            ...
            NetTool netTool = UnityEngine.Object.FindObjectOfType<ToolController>().gameObject.GetComponent<NetTool>();
            ChangeElevation_Iterator changeElevation_Iterator = new ChangeElevation_Iterator();
            ...
            public bool MoveNext()
            {
                ...
                switch (num)
                {
                    case 0u:
                        ...
                        if (this.info != null)
                        {
                            GetAdjustedElevationLimits(this.info.m_netAI, out this.min, out this.max);
                            if (this.max > this.min)
                            {
                                this.elevation = Mathf.Clamp(Mathf.Clamp((int)m_elevation.GetValue(parentTool), this.min, this.max) + this.delta, this.min, this.max);
                                if (this.elevation != (int)m_elevation.GetValue(parentTool))
                                {
                                    m_elevation.SetValue(parentTool, this.elevation);
                                    this.result = true;
                                }
                            }
                        }
